...
namespace Yarn.Unity.Example {
    ...
    public class VNManager : DialogueViewBase
    {
		...
		public List<Actor> loadActors = new List<Actor>();
		...
		void Awake () {
			...
			if ( useResourcesFolders ) {
				...
				var allActorsInResources = Resources.LoadAll<Actor>("");
				loadActors.AddRange(allActorsInResources);
			}
		}
		...
		public void SetActor(string spriteName, string positionX = "", string positionY = "", string colorHex = "")
		{
			...
			{ // missing 2 params, override both x and y
				newPos = actors[spriteName].rectTransform.anchoredPosition;
			}
			else if (positionY == string.Empty)
			{ // missing 1 param, override y
				newPos.y = actors[spriteName].rectTransform.anchoredPosition.y;
			}
			// if any missing color params, then assume actor color
			// should stay the same
			if (colorHex == string.Empty)
			{
				actorColor = actors[spriteName].actorColor;
			}
			newActor.rectTransform.anchoredPosition = newPos;
			// clean-up
			Destroy(actors[spriteName].gameObject);
			actors.Remove(spriteName);
			actors.Remove(spriteName);

			// save actor data
			actors.Add(spriteName, new VNActor(newActor, actorColor));
		}
		...
		public void SetActor(string actorName, string spriteName, string positionX = "", string positionY = "", string colorHex = "" ) {
			...
			// have to use SetSprite() because par[2] and par[3] might be
			// keywords (e.g. "left", "right")
			var newActor = SetSpriteUnity(actorName, spriteName, positionX, positionY);
		...
		public Image SetSpriteUnity(string actorName, string spriteName, string positionX = "", string positionY = "")
		{
			...
			// actually instantiate and draw sprite now
			return SetSpriteActual(actorName, spriteName, pos);
		}
		...
		Image SetSpriteActual(string actorName, string spriteName, Vector2 position)
		{
			var newSpriteObject = Instantiate<Image>(genericSprite, genericSprite.transform.parent);
			sprites.Add(newSpriteObject);
			newSpriteObject.name = spriteName;
			newSpriteObject.sprite = FetchAsset<Actor>(actorName).GetSprite(spriteName);
			newSpriteObject.SetNativeSize();
			newSpriteObject.rectTransform.anchoredPosition = Vector2.Scale(position, screenSize);
			return newSpriteObject;
		}
		...
		T FetchAsset<T>( string assetName ) where T : UnityEngine.Object {
			...
			{
				foreach (var actor in loadActors)
				{
					if (actor.name == assetName)
					{
						return actor as T;
					}
				}
